...
class PiPyNode(AdmissionMixin, RpcMixin):
    ...
    async def initialize(self, *, wifi_ssid: str | None = None, wifi_password: str | None = None) -> None:
        ...
        if not self.cfg.enrolled:
            self.cfg = await asyncio.to_thread(
                genesis,
                self.paths,
                self.store,
                self.cfg,
    ...
    async def start(self) -> None:
        self.server = await asyncio.start_server(
            self._handle_connection, host="0.0.0.0", port=self.cfg.port
        )
        self._tasks.extend(
            [
                ...
                asyncio.create_task(
                    TelemetryBroadcaster(self.telemetry_payload).run(self.stop_event),
                    name="telemetry",
                ),
            ]
        )
        if self.cfg.bootstrap_bundle and not self.cfg.enrolled:
            self._tasks.append(
                asyncio.create_task(self._join_from_bundle_loop(), name="admission")
            )
    ...
    async def _refresh_self_member(self) -> None:
        ...
        member = Member(
            self.cfg.node_id,
            self.cfg.node_public,
            self._advertised_host(),
            self.cfg.port,
            socket.gethostname(),
            current.joined_at if current else now,
            now,
            NodeStatus.ALIVE,
            current.capacity if current else 1.0,
        )
    ...
    async def telemetry_payload(self) -> dict[str, Any]:
        ...
        active = next(
            (job for job in reversed(jobs) if job.status in {JobStatus.PENDING, JobStatus.RUNNING}),
            None,
        )
        ...
        return {
            ...
            "work_complete": sum(1 for unit in work if unit.status == WorkStatus.COMPLETE),
            "work_total": len(work),
        }
    ...
    async def _heartbeat_loop(self) -> None:
        while not self.stop_event.is_set():
            if self.cfg.enrolled:
                await self._refresh_self_member()
                self_member = await self._member_for(self.cfg.node_id)
                for member in await asyncio.to_thread(self.store.members):
                    if member.node_id == self.cfg.node_id:
                        continue
                    try:
                        response = await request(
                            ...
                            self._signed(
                                ...
                                host=self._advertised_host(),
                                port=self.cfg.port,
                                capacity=self_member.capacity if self_member else 1.0,
                            ),
                            timeout=1.5,
                        )
                        if response.get("kind") == "ok":
        ...
        while not self.stop_event.is_set():
            if self.cfg.enrolled:
                ...
                for member in members:
                    ...
                    await asyncio.to_thread(self.store.put_member, member)
                new_leader = min((member.node_id for member in alive), default=self.cfg.node_id)
    ...
    async def leader_endpoint(self) -> dict[str, Any] | None:
        ...
        return (
            None
            if member is None
            else {"node_id": member.node_id, "host": member.host, "port": member.port}
        )
    ...
    async def submit_job(self, kind: str, parameters: dict[str, Any]) -> Job:
        ...
        response = await request(
            endpoint["host"],
            int(endpoint["port"]),
            self._signed("submit_job", job_kind=kind, parameters=parameters),
            timeout=10.0,
        )
    ...
    async def _lease_work(self, worker_id: str) -> WorkUnit | None:
        ...
        for unit in await asyncio.to_thread(self.store.work):
            ...
            if unit.status == WorkStatus.AVAILABLE:
                unit.status = WorkStatus.LEASED
                unit.lease_owner = worker_id
                unit.lease_until = now + DEFAULT_LEASE_SECONDS
                await asyncio.to_thread(self.store.put_work, unit)
                return unit
        return None
    ...
    async def _accept_result(
        ...
        if work and all(
            unit.status == WorkStatus.COMPLETE and unit.result is not None for unit in work
        ):
            job.result = await asyncio.to_thread(
                implementation.reduce,
                job.parameters,
                [unit.result for unit in work if unit.result is not None],
            )
                ...
                try:
                    ...
                    if unit:
                        ...
                        if job:
                            ...
                            await self._record_capacity(
                                1.0 / max(1e-6, time.perf_counter() - started)
                            )
                            if await self.is_leader():
        ...
        tasks = [
            asyncio.create_task(
                request(
                    member.host,
                    member.port,
                    self._signed("snapshot", snapshot=snapshot),
                    timeout=3.0,
                )
            )
            for member in await asyncio.to_thread(self.store.members)
            if member.node_id != self.cfg.node_id and member.status != NodeStatus.DEAD
        ]
        if tasks:
