...
class AdmissionMixin:
    async def _handle_admission(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter, hello: dict[str, Any]) -> None:
        if not self.cfg.enrolled or not self.cfg.cluster_private or not self.cfg.cluster_public or not self.cfg.unity_key:
            await write_message(writer, message("admission_reject", error="node cannot admit members"))
            return
        token_id = str(hello.get("token_id") or "")
        secret = str(hello.get("secret") or "")
        records = {r["token_id"]: r for r in await asyncio.to_thread(self.store.enrollments)}
        record = records.get(token_id)
        if not record or record["used"] or record["token_hash"] != token_hash(secret):
            await write_message(writer, message("admission_reject", error="invalid or used enrollment capability"))
            return
        unsigned = {k: v for k, v in hello.items() if k != "signature"}
        if not verify(str(hello["node_public"]), unsigned, str(hello.get("signature") or "")):
            await write_message(writer, message("admission_reject", error="invalid node identity proof"))
            return
        ...
        proof_payload = decrypt(session_key, dict(proof["envelope"]), b"pipy-admission-proof")
        if proof_payload.get("token_id") != token_id or proof_payload.get("server_nonce") != nonce.hex():
            raise ProtocolError("invalid admission proof")
        if not await asyncio.to_thread(self.store.consume_enrollment, token_id, token_hash(secret)):
        ...
        await self._replicate_unity_rotation(old_unity, new_unity, exclude={member.node_id})
        certificate_body = {"cluster_id": self.cfg.cluster_id, "node_id": member.node_id, "node_public": member.public_key, "joined_at": now}
        admission = {
            "cluster_id": self.cfg.cluster_id,
            ...
            "membership_certificate": {"body": certificate_body, "signature": sign(self.cfg.cluster_private, certificate_body)},
            "snapshot": (await asyncio.to_thread(self.store.snapshot)).to_dict(),
        }
        await write_message(writer, message("admission_grant", envelope=encrypt(session_key, admission, b"pipy-admission-grant")))
        ack = await read_message(reader)
        if ack["kind"] != "admission_ack":
            raise ProtocolError("expected admission acknowledgment")
        ack_payload = decrypt(session_key, dict(ack["envelope"]), b"pipy-admission-ack")
        if ack_payload.get("node_id") != member.node_id:
            raise ProtocolError("invalid admission acknowledgment")
        await write_message(writer, message("admission_complete", cluster_id=self.cfg.cluster_id))
    ...
    async def _join_from_bundle_loop(self) -> None:
        ...
        while not self.cfg.enrolled and not self.stop_event.is_set():
            ...
            if bundle.get("seed_host"):
                candidates.append((str(bundle["seed_host"]), int(bundle.get("seed_port", self.cfg.port))))
            candidates.extend((m.host, m.port) for m in await asyncio.to_thread(self.store.members) if m.node_id != self.cfg.node_id)
    ...
    async def _join_member(self, host: str, port: int, bundle: dict[str, Any]) -> None:
        ...
        try:
            ...
            hello["signature"] = sign(self.cfg.node_private, {k: v for k, v in hello.items() if k != "signature"})
            ...
            if challenge["cluster_id"] != bundle["cluster_id"] or not verify(str(bundle["cluster_public"]), transcript, str(challenge["cluster_signature"])):
                raise RuntimeError("cluster identity verification failed")
            session_key = eph.derive(str(challenge["member_ephemeral"]), salt=bytes.fromhex(str(challenge["server_nonce"])))
            proof = {"token_id": bundle["token_id"], "server_nonce": challenge["server_nonce"], "node_id": self.cfg.node_id}
            await write_message(writer, message("admission_proof", envelope=encrypt(session_key, proof, b"pipy-admission-proof")))
            grant = await read_message(reader)
            if grant["kind"] != "admission_grant":
                raise RuntimeError(grant.get("error", "admission grant missing"))
            admission = decrypt(session_key, dict(grant["envelope"]), b"pipy-admission-grant")
            cert = admission["membership_certificate"]
            if not verify(str(bundle["cluster_public"]), cert["body"], cert["signature"]):
            ...
            self.cfg.save(self.paths.config)
            await asyncio.to_thread(self.store.apply_snapshot, admission["snapshot"])
            await self._refresh_self_member()
            await write_message(writer, message("admission_ack", envelope=encrypt(session_key, {"node_id": self.cfg.node_id}, b"pipy-admission-ack")))
            if (await read_message(reader))["kind"] != "admission_complete":
    ...
    async def _replicate_unity_rotation(self, old_key: str, new_key: str, *, exclude: set[str]) -> None:
        body = {"new_unity_key": new_key, "epoch": int(await asyncio.to_thread(self.store.get, "epoch", 0)) + 1, "issued_by": self.cfg.node_id}
        tasks = []
        for member in await asyncio.to_thread(self.store.members):
            if member.node_id == self.cfg.node_id or member.node_id in exclude:
                continue
            tasks.append(asyncio.create_task(request(member.host, member.port, self._signed("unity_rotate", body=body, mac=mac(old_key, body)), timeout=3.0)))
        if tasks:
